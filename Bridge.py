...
class MS_Init_ImportProcess():
        ...
        try:
            ...
            if globals()['Megascans_DataSet'] != None:
                ...
                for js in self.json_Array:
                    ...
                    actual_fps = bpy.context.scene.render.fps
                    
                    self.initImportProcess()
                    print("Imported asset from " + self.assetName + " Quixel Bridge")
                    bpy.context.scene.render.fps = actual_fps
    ...
    def SetupMaterial (self):
        if "albedo" in self.textureTypes:
            if "ao" in self.textureTypes:
                self.CreateTextureMultiplyNode("albedo", "ao", -250, 600, -640, 660, -640, 370, 0, 1, True, 0)
            else:
                self.CreateTextureNode("albedo", -640, 610, 0, True, 0)
        
        if self.isSpecularWorkflow:
            if "specular" in self.textureTypes:
                self.CreateTextureNode("specular", -640, 80, 0, True, 12)
            
            if "gloss" in self.textureTypes:
                glossNode = self.CreateTextureNode("gloss", -640, 370)
                invertNode = self.CreateGenericNode("ShaderNodeInvert", -250, 230)
                ...
                self.mat.node_tree.links.new(self.nodes.get(self.parentName).inputs[2], invertNode.outputs[0])
            elif "roughness" in self.textureTypes:
                self.CreateTextureNode("roughness", -640, 80, 1, True, 2)
        else:
        ...
        if "opacity" in self.textureTypes:
            opacityNode = self.CreateTextureNode("opacity", -940, 170, 1, True, 4)
            self.mat.blend_method = 'HASHED'

        if "translucency" in self.textureTypes:
            # Create Translucent BSDF node.
            translucent_node = self.nodes.new('ShaderNodeBsdfTranslucent') 
            translucent_node.location = (100, -50)
            # Create Mix Shader node.
            mix_node = self.nodes.new('ShaderNodeMixShader')
            mix_node.location = (300, 130)
            # Add translucent_node to mix_node connection
            self.mat.node_tree.links.new(mix_node.inputs[2], translucent_node.outputs[0])
            # Connect Principled BSDF to the Mix Shader node.
            self.mat.node_tree.links.new(mix_node.inputs[1], self.nodes.get(self.parentName).outputs[0])
           
            translucency_node = self.CreateTextureNode("translucency", -940, -120)
            # Add translucency_node to translucent_node connection
            self.mat.node_tree.links.new(translucent_node.inputs['Color'], translucency_node.outputs[0])
            # Connect Mix Shader node to the material output.
            self.mat.node_tree.links.new(self.nodes.get(self.materialOutputName).inputs[0], mix_node.outputs[0])
            # Connect Opacity to the Mix Shader node.
            self.mat.node_tree.links.new(mix_node.inputs[0], opacityNode.outputs['Color'])
            
            
        elif "transmission" in self.textureTypes:
            self.CreateTextureNode("transmission", -940, -120, 1, True, 17)

        # If HIGH POLY selected > use normal_bump and no displacement
        # If LODs selected > use corresponding LODs normal + displacement
        if self.isHighPoly:
            self.BumpSetup = False
        self.CreateNormalNodeSetup(True, 19 if bpy.app.version < (2, 91, 0) else 5)
    ...
    def CreateMaterial(self):
        ...
        self.mat.node_tree.nodes[self.parentName].inputs[1].default_value = 1 if self.isMetal else 0 # Metallic value
        self.mat.node_tree.nodes[self.parentName].inputs[3].default_value = self.IOR
        
        self.mappingNode = None

        if self.assetType not in ["3d", "3dplant"]:
            # Create mapping node.
            self.mappingNode = self.CreateGenericNode("ShaderNodeMapping", -1400, 0)
            self.mappingNode.vector_type = 'TEXTURE'
            # Create texture coordinate node.
            texCoordNode = self.CreateGenericNode("ShaderNodeTexCoord", -1750, 0)
            # Connect texCoordNode to the mappingNode
            self.mat.node_tree.links.new(self.mappingNode.inputs[0], texCoordNode.outputs[2])
            #Create Value node to control scale factor
            valueNode = self.CreateGenericNode("ShaderNodeValue", -1600, -250)
            valueNode.outputs['Value'].default_value = 1
            #Connect valueNode to the mappingNode
            self.mat.node_tree.links.new(self.mappingNode.inputs['Scale'], valueNode.outputs['Value'])
    ...
    def CreateTextureMultiplyNode(self, aTextureType, bTextureType, PosX, PosY, aPosX, aPosY, bPosX, bPosY, aColorspace, bColorspace, connectToMaterial, materialInputIndex):
        ...
        multiplyNode.inputs['Factor'].default_value = 1
        #Setup A and B nodes
        ...
        self.mat.node_tree.links.new(multiplyNode.inputs[6], textureNodeA.outputs[0])
        self.mat.node_tree.links.new(multiplyNode.inputs[7], textureNodeB.outputs[0])


        if connectToMaterial:
            self.mat.node_tree.links.new(self.nodes.get(self.parentName).inputs[0], multiplyNode.outputs[2])
    ...
    def CreateNormalNodeSetup(self, connectToMaterial, materialInputIndex):
        ...
        if self.NormalSetup and self.BumpSetup:
            ...
            normalNode = self.CreateGenericNode("ShaderNodeNormalMap", -250, -210)
            # Add normalMapNode to normalNode connection
            self.mat.node_tree.links.new(normalNode.inputs[1], normalMapNode.outputs[0])
    ...
    def CreateDisplacementSetup(self, connectToMaterial):
        if self.DisplacementSetup == "adaptive":
            ...
            displacementNode = self.CreateGenericNode("ShaderNodeDisplacement", -250, -500)
            ...
            displacementMapNode = self.CreateTextureNode("displacement", -640, -500)
            # Add displacementMapNode to RGBSplitterNode connection
            self.mat.node_tree.links.new(displacementNode.inputs[0], displacementMapNode.outputs[0])
            '''
            # Add RGBSplitterNode to displacementNode
            connection
